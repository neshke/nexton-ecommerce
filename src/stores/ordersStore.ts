...
export const useOrdersStore = defineStore('orders', () => {
  ...
  async function fetchUserOrders(): Promise<void> {
    ...
    try {
      ...
      // Provera da li API vraća status unutar data objekta ili se oslanjamo na HTTP status
      // Ovaj kod pretpostavlja da API vraća { status: 200, data: [], message: "..." } ili slično
      // Ako API vraća direktno podatke ili samo poruku o grešci, ovo treba prilagoditi
      if (response.data && (response.data.status === 200 || response.status === 200) && response.data.data) {
        ...
        console.warn('Neočekivana struktura podataka ili greška iz API-ja za porudžbine:', response.data);
